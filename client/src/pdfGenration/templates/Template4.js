import React from "react";
import PHONE from "../images/phone.png";
import EMAIL from "../images/email.png";
import ADDRESS from "../images/address.png";
import {
  Page,
  Text,
  View,
  Document,
  StyleSheet,
  Font,
  Image,
} from "@react-pdf/renderer";

Font.register({
  family: "Nunito",
  src:
    "https://cdn.jsdelivr.net/npm/@expo-google-fonts/nunito@0.1.0/Nunito_700Bold.ttf",
});
Font.register({
  family: "ariel",
  src:
    "https://cdn.jsdelivr.net/npm/bpg-arial@1.0.0/fonts/bpg-arial-webfont.ttf",
});

const colorDarkBlue = "#011B2E";
const colorWhiteShade = "#F2F2F2";
const colorDarkRed = "#B71C1C";
// Create styles
const styles = StyleSheet.create({
  page: {
    width: "100%",
    fontFamily: "Nunito",
    paddingVertical: 25,
    color: "#323232",
  },
  header: {
    backgroundColor: colorDarkBlue,
    minHeight: "90px",
    width: "100%",
    paddingVertical: 15,
    marginTop: -25,
  },
  dot: {
    height: "5px",
    width: "5px",
    backgroundColor: "#000",
    marginRight: 8,
  },
  heading: {
    cap: {
      fontSize: 12,
      backgroundColor: colorDarkBlue,
      marginBottom: 3,
      color: "#fff",
      paddingVertical: 3,
      paddingHorizontal: 5,
    },
    nor: {
      fontSize: 15,
      color: colorDarkBlue,
      borderBottom: 2,
      borderBottomColor: colorDarkBlue,
      width: "100%",
    },
  },
  flexBox: {
    display: "flex",
    flexDirection: "row",
    paddingHorizontal: 15,
  },

  section: {
    left: {
      width: "65%",
      fontSize: 15,
      paddingLeft: 25,
      paddingTop: 15,
    },
    right: {
      width: "35%",
      fontSize: 15,
      backgroundColor: colorDarkRed,
      marginTop: -105,
      paddingVertical: 25,
      paddingHorizontal: 15,
      color: colorWhiteShade,
    },
  },

  basic: {
    wrapper: {},
    name: {
      fontSize: 18,
      color: colorWhiteShade,
    },
    proffesion: {
      fontSize: 13,
      paddingLeft: 3,
      marginVertical: 2,
      color: colorDarkBlue,
    },
    pitch: {
      width: "100%",
      fontSize: 9,
      paddingLeft: 3,
      color: colorWhiteShade,
      textAlign: "justify",
    },
  },
  spacer: {
    width: "35%",
  },
  contact: {
    wrapper: {
      width: "60%",
      display: "flex",
      alignItems: "flex-start",
      paddingTop: 25,
      paddingLeft: 20,
    },
    text: {
      fontSize: 9,
      width: "50%",
      color: colorWhiteShade,
    },
    flexBox: {
      display: "flex",
      flexDirection: "row",
      alignItems: "center",
      paddingVertical: 5,
      paddingHorizontal: 5,
    },
  },
  icon: {
    address: {
      marginRight: 7,
      width: "4%",
    },
    email: {
      marginRight: 7,
      width: "3%",
    },
    phone: {
      marginRight: 7,
      width: "3%",
    },
  },
  skills: {
    wrapper: {
      marginTop: 20,
    },
    flexBox: {
      display: "flex",
      flexDirection: "row",
      alignItems: "center",
      paddingVertical: 10,
      paddingHorizontal: 5,
    },
    left: {
      width: "30%",
    },
    right: {
      width: "70%",
      marginHorizontal: 5,
    },
    text: { fontSize: 9, color: colorWhiteShade },
    barBorder: {
      width: "90%",
      height: "4px",
      backgroundColor: colorWhiteShade,
    },
  },
  education: {
    wrapper: { paddingBottom: 20 },
    flexBox: {
      display: "flex",
      flexDirection: "row",
      alignItems: "center",
      paddingTop: 10,
      paddingLeft: 15,
    },
    institue: { fontSize: 11, color: colorDarkBlue },
    course: { fontSize: 9.5, color: colorDarkRed },
    date: { fontSize: 10, color: colorDarkRed },
    left: { width: "80%", paddingRight: 20 },
    right: { width: "20%" },
  },
  experience: {
    wrapper: { paddingBottom: 20 },
    flexBox: {
      display: "flex",
      flexDirection: "row",
      alignItems: "center",
      paddingTop: 10,
      paddingLeft: 15,
    },
    org: { fontSize: 11, color: colorDarkBlue },
    position: { fontSize: 9.5, color: colorDarkRed },
    date: { fontSize: 9, color: colorDarkRed },
    left: { width: "70%", paddingRight: 20 },
    right: { width: "30%" },
    box: {},
    to: {
      fontSize: 9,
      paddingLeft: 35,
    },
    workDetailsBox: {
      paddingLeft: 20,
    },
    workDetail: {
      display: "flex",
      flexDirection: "row",
      alignItems: "center",
    },
    workDetailText: {
      fontSize: 10,
    },
  },
  diploma: {
    wrapper: { paddingBottom: 20 },
    flexBox: {
      display: "flex",
      flexDirection: "row",
      alignItems: "center",
      paddingTop: 10,
      paddingLeft: 15,
    },
    institue: { fontSize: 11, color: colorDarkBlue },
    course: { fontSize: 9.5, color: colorDarkRed },
    date: { fontSize: 10, color: colorDarkRed },
    left: { width: "70%", paddingRight: 20 },
    right: { width: "30%" },
  },
});

const Template4 = ({ resumeInfo }) => {
  const {
    firstname,
    lastname,
    contact_details,
    address,
    profession,
    discription,
    academic_qualification,
    experience,
    fresher,
    certificates_diplomas,
    skills,
  } = resumeInfo;
  return (
    <Document>
      <Page style={styles.page}>
        <View style={styles.header}>
          <View style={styles.flexBox}>
            <View style={styles.spacer}></View>
            <View style={styles.contact.wrapper}>
              <View style={styles.contact.flexBox}>
                <Image source={EMAIL} style={styles.icon.email} />
                <Text style={styles.contact.text}>{contact_details.email}</Text>
              </View>
              <View style={styles.contact.flexBox}>
                <Image source={PHONE} style={styles.icon.phone} />
                <Text style={styles.contact.text}>
                  {contact_details.mobile}
                </Text>
              </View>
              <View style={styles.contact.flexBox}>
                <Image source={ADDRESS} style={styles.icon.address} />
                <Text style={styles.contact.text}>
                  {address.line_1 +
                    " " +
                    address.line_2 +
                    " " +
                    address.line_3 +
                    " " +
                    address.city +
                    " " +
                    address.state}
                </Text>
              </View>
            </View>
          </View>
        </View>
        <View style={styles.flexBox}>
          <View style={styles.section.right}>
            <View style={styles.basic.wrapper}>
              <Text break style={styles.basic.name}>
                {firstname + " " + lastname}
              </Text>
              <Text style={styles.basic.proffesion}>{profession}</Text>
              <Text style={styles.basic.pitch}>{discription}</Text>
            </View>
            {skills.length !== 0 && (
              <View style={styles.skills.wrapper}>
                <Text style={styles.heading.cap}>Skills</Text>

                {skills.map((data, index) => {
                  return (
                    <View style={styles.skills.flexBox} key={index}>
                      <View style={styles.skills.left}>
                        <Text style={styles.skills.text}>{data.skill} :</Text>
                      </View>
                      <View style={styles.skills.right}>
                        <View style={styles.skills.barBorder}>
                          <View
                            style={{
                              width: `${data.skill_level}%`,
                              height: "100%",
                              backgroundColor: colorDarkBlue,
                            }}
                          ></View>
                        </View>
                      </View>
                    </View>
                  );
                })}
              </View>
            )}
          </View>
          <View style={styles.section.left}>
            {academic_qualification.length !== 0 && (
              <View style={styles.education.wrapper}>
                <Text style={styles.heading.nor}>Education</Text>
                {academic_qualification.map((qualification, index) => {
                  return (
                    <View key={index} style={styles.education.flexBox}>
                      <View style={styles.education.left}>
                        <Text style={styles.education.institue}>
                          {qualification.institute}
                        </Text>
                        <Text style={styles.education.course}>
                          {qualification.qualification_title}
                        </Text>
                      </View>
                      <View style={styles.education.right}>
                        <Text style={styles.education.date}>
                          {qualification.passing_year}
                        </Text>
                      </View>
                    </View>
                  );
                })}
              </View>
            )}
            {certificates_diplomas.length !== 0 && (
              <View style={styles.diploma.wrapper}>
                <Text style={styles.heading.nor}>Diploma/Certificate</Text>
                {certificates_diplomas.map((dips, index) => {
                  return (
                    <View key={index} style={styles.diploma.flexBox}>
                      <View style={styles.diploma.left}>
                        <Text style={styles.diploma.institue}>
                          {dips.diploma_institute}
                        </Text>
                        <Text style={styles.diploma.course}>
                          {dips.diploma_title}
                        </Text>
                      </View>
                      <View style={styles.diploma.right}>
                        <Text style={styles.diploma.date}>
                          {dips.date_of_completion}
                        </Text>
                      </View>
                    </View>
                  );
                })}
              </View>
            )}
            {experience.length !== 0 && !fresher && (
              <View style={styles.experience.wrapper}>
                <Text style={styles.heading.nor}>Experience</Text>
                {experience.map((exp, index) => {
                  return (
                    <View key={index} style={styles.experience.box}>
                      <View style={styles.experience.flexBox}>
                        <View style={styles.experience.left}>
                          <Text style={styles.experience.org}>
                            {exp.organisation}
                          </Text>
                          <Text style={styles.experience.position}>
                            {exp.position}
                          </Text>
                        </View>
                        <View style={styles.experience.right}>
                          <Text style={styles.experience.date}>
                            {exp.joining_month + ", " + exp.joining_year}
                          </Text>
                          <Text style={styles.experience.to}>to</Text>
                          <Text style={styles.experience.date}>
                            {exp.leaving_month + ", " + exp.leaving_year}
                          </Text>
                        </View>
                      </View>
                      <View style={styles.experience.workDetailsBox}>
                        {exp.work_details.map((data, index) => {
                          return (
                            <View
                              key={index}
                              style={styles.experience.workDetail}
                            >
                              <View style={styles.dot}></View>
                              <Text style={styles.experience.workDetailText}>
                                {data.workDetail}
                              </Text>
                            </View>
                          );
                        })}
                      </View>
                    </View>
                  );
                })}
              </View>
            )}
          </View>
        </View>
      </Page>
    </Document>
  );
};

export default Template4;
